...
const PreferencesScreen = () => {
	...
	const onPressCategory = (category: Topic, idx: number) => {
		// const index = selectedTopics.findIndex((d) => d.topic == category.topic);
		// // Reactotron.log(index);

		// const newData = [...selectedTopics];
		// Reactotron.log("newData1:", newData);
		// if (index == -1) {
		// 	newData.push(category);

		// 	for (const obj of newData) {
		// 		obj["selected"] = true;
		// 		Reactotron.log("obj.selected:", obj);
		// 	}
		// 	// Reactotron.log("category:", category);
		// 	// newData.forEach((obj) => {
		// 	// 	obj.selected = true;
		// 	// });

		// 	Reactotron.log("newData2:", newData);
		// 	setSelectedTopics(newData);
		// } else {
		// 	newData.pop(category);
		// 	Reactotron.log("newData3:", newData);
		// 	setSelectedTopics(newData);
		// }
	...
	return (
		<View style={styles.container}>
				...
				renderItem={({ item, index }) => {
					return (
						...
						>
							...
							{isSelected(item) && <Ionicons name="ios-checkmark-sharp" size={wp(16)} color={COLORS.RED_COLOR} />}
						</TouchableOpacity>
					);
				}}
			...
			<Button
				...
				onPress={() => {
					dispatch(setUserSelectedTopics(selectedTopics));
				}}
			/>
			<SnackBar
